...
class DinoVisionTransformer(Model):
        ...
        **kwargs,
        ...
        self.register_tokens = (
            self.add_weight(
                ...
                shape=(1, num_register_tokens, embed_dim),
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        N = ops.shape(self.pos_embed)[1] - 1
        if npatch == N and w == h:
            return self.pos_embed

        pos_embed = ops.cast(self.pos_embed, "float32")
        ...
        patch_pos_embed = ops.reshape(patch_pos_embed, (1, M, M, dim))
        ...
        patch_pos_embed = ops.slice(patch_pos_embed, [0, 0, 0, 0], [1, h0, w0, dim])
        patch_pos_embed = ops.reshape(patch_pos_embed, (1, -1, dim))
        return ops.cast(ops.concatenate([class_pos_embed, patch_pos_embed], axis=1), previous_dtype)

    def prepare_tokens_with_masks(self, x, masks=None):
        B, H, W, C = ops.shape(x)
        x = self.patch_embed(x)
        if masks is not None:
            mask_token_expanded = ops.broadcast_to(self.mask_token, (B, ops.shape(x)[1], self.embed_dim))
            x = ops.where(ops.expand_dims(masks, -1), mask_token_expanded, x)
        cls_tok = ops.broadcast_to(self.cls_token, (B, 1, self.embed_dim))
        x = ops.concatenate([cls_tok, x], axis=1)
        x = ops.add(x, self.interpolate_pos_encoding(x, H, W))
        if self.register_tokens is not None:
            reg_tokens = ops.broadcast_to(self.register_tokens, (B, self.num_register_tokens, self.embed_dim))
    ...
    def call(self, x, training=None, masks=None):
        ...
        if training:
            ...
            return self.head(final_output["x_norm_clstoken"])
