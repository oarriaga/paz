...
class Block(keras.Layer):
    ...
    def call(self, x, training=None):
        def attn_residual_func(x):
            return self.ls1(self.attn(self.norm1(x), training=training), training=training)

        def ffn_residual_func(x):
            return self.ls2(self.mlp(self.norm2(x), training=training), training=training)

        is_training = training if training is not None else self.training
        if is_training and self.sample_drop_ratio > 0.1:
        ...
        elif is_training and self.sample_drop_ratio > 0.0:
