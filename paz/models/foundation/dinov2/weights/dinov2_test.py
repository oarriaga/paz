...
# Torch-hub parity tests are compute/network heavy: gated behind --run-heavy.
...
class TestFixtures:
    ...
    @classmethod
    def get_keras_model(cls, keras_path: str, variant: str):
        """Build from source and load_weights by name (single-entry cache).

        load_weights is version-robust; load_model fails to deserialize the
        saved op-graph across mismatched keras versions.
        """
        _evict_other_entries(cls._keras_models, keras_path)
        if keras_path not in cls._keras_models:
            model = VARIANT_BUILDERS[variant](img_size=DEFAULT_INPUT_SIZE)
            model.load_weights(keras_path)
            cls._keras_models[keras_path] = model
...
def keras_model(model_config):
    ...
    return TestFixtures.get_keras_model(
        model_config["keras_path"], model_config["variant"]
    )
