...
class Attention(Layer):
    def __init__(
        ...
        **kwargs,
        ...
        self.predict_query_key_value = Dense(self.dimension * 3, use_bias=self.use_qkv_bias, name="qkv", **kwargs)
        self.projection_layer = Dense(self.dimension, use_bias=self.use_projection_bias, name="proj", **kwargs)
    ...
    def call(self, x, attention_bias=None, training=None):
        ...
        query_key_value = ops.reshape(query_key_value, (batch_size, num_tokens, 3, self.num_heads, self.head_dimension))
        ...
        attention = ops.matmul(query, ops.transpose(key, axes=[0, 1, 3, 2])) * self.scale
