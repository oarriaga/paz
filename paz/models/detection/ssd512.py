...
def SSD512(num_classes=81, weights='COCO',
           input_shape=(512, 512, 3),
           num_priors=[4, 6, 6, 6, 6, 4, 4],
           return_base=False, l2_regularization=0.0005,
           trainable_base=True, batch_norm=False, model_name='SSD512'):

    """Single-shot-multibox detector for 512x512 inputs
    # Arguments
    ...
    conv8_2 = Conv2D(256, (3, 3), padding='valid', strides=(2, 2),
                     ...
                     kernel_regularizer=l2(l2_regularization))(conv8_1z)
    ...
    conv9_2 = Conv2D(256, (3, 3), padding='valid', strides=(2, 2),
                     ...
                     kernel_regularizer=l2(l2_regularization))(conv9_1z)
    ...
    conv10_2 = Conv2D(256, (4, 4), padding='valid', strides=(1, 1),
                      ...
                      kernel_regularizer=l2(l2_regularization))(conv10_1z)

    ssd_tensors = [conv4_3_norm, fc7, conv6_2, conv7_2,
                   conv8_2, conv9_2, conv10_2]
    if return_base:
        output_tensor = ssd_tensors

    else:
        output_tensor = create_multibox_head(
            ssd_tensors, num_classes, num_priors,
            with_batch_norm=batch_norm, l2_regularization=l2_regularization)

    model = Model(inputs=image, outputs=output_tensor, name=model_name)

    if weights is not None:
        model_name = '_'.join([model_name, weights])
