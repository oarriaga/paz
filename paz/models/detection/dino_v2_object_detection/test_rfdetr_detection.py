...
needs_pt = pytest.mark.skipif(not HAS_PT, reason="Reference library not installed")  # fmt: skip

# ── Constants ─────────────────────────────────────────────────────────────────
_CACHE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".test_cache")  # fmt: skip
...
def _assert_top_k_close(
    ...
    k_idx = np.lexsort((ks["boxes"][:n, 1], ks["boxes"][:n, 0], ks["labels"][:n]))  # fmt: skip
    p_idx = np.lexsort((ps["boxes"][:n, 1], ps["boxes"][:n, 0], ps["labels"][:n]))  # fmt: skip
    ...
    np.testing.assert_allclose(ks["boxes"], ps["boxes"], atol=box_atol, err_msg="boxes")  # fmt: skip
...
def test_all_detection_variants_are_rfdetr_subclass():
    for name in ALL_NAMES:
        assert VARIANTS[name]["keras"] in K_REGISTRY.values()
...
def test_same_expected_class_nano(keras_nano, pt_nano, coco_images, img_name):
    expected = COCO_IMAGES[img_name]["expected_classes"]
    k_labels = set(
        keras_nano.predict(coco_images[img_name], threshold=0.3)[0]["labels"].tolist()  # fmt: skip
    )
    p_labels = set(
...
@needs_pt
@pytest.mark.parametrize("img_name", list(COCO_IMAGES.keys()))
def test_detection_count_similar_nano(keras_nano, pt_nano, coco_images, img_name):  # fmt: skip
    k_n = len(keras_nano.predict(coco_images[img_name], threshold=0.5)[0]["scores"])  # fmt: skip
...
def test_variant_expected_classes(variant, coco_images):
    ...
    try:
        for img_name, img in coco_images.items():
            expected = COCO_IMAGES[img_name]["expected_classes"]
            k_set = set(k_model.predict(img, threshold=0.3)[0]["labels"].tolist())  # fmt: skip
            p_set = set(pt_model.predict(img, threshold=0.3).class_id.tolist())
            assert (
                expected & k_set
            ), f"Keras {variant}/{img_name}: {expected} vs {k_set}"
            assert expected & p_set, f"PT {variant}/{img_name}: {expected} vs {p_set}"  # fmt: skip
    finally:
        del k_model, pt_model
        gc.collect()
