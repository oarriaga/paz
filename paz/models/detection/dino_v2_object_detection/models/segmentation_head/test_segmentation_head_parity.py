...
def set_seed(seed=42):
    """Set random seeds across NumPy, PyTorch, and Keras."""
    np.random.seed(seed)
...
def test_depthwise_conv_block():
    """Verify DepthwiseConvBlock output matches the reference."""
    set_seed()
    dim = 64
    ...
    # Build Keras block with a dummy forward pass
...
def test_mlp_block():
    """Verify MLPBlock output matches the reference."""
    set_seed()
    dim = 64
...
def test_segmentation_head():
    """Verify full SegmentationHead forward pass matches the reference."""
    set_seed()
    in_dim = 64
    num_blocks = 2
    ...
    # Build Keras head with dummy inputs
...
def test_point_sample():
    """Verify point_sample matches the reference grid_sample wrapper."""
    set_seed()
    N, C, H, W = 2, 3, 32, 32
    P = 100
...
def test_uncertainty():
    """Verify calculate_uncertainty matches the reference."""
...
def test_get_uncertain_points():
    """Verify shape and range of uncertainty-sampled point coordinates."""
    set_seed()
    N, C, H, W = 2, 1, 32, 32
    num_points = 50
...
def test_segmentation_head_bottleneck_none():
    """Verify SegmentationHead with bottleneck_ratio=None."""
    set_seed()
    in_dim = 32
    num_blocks = 1
...
def test_segmentation_head_skip_blocks():
    """Verify SegmentationHead with skip_blocks=True."""
    set_seed()
    in_dim = 32
    num_blocks = 2
    ...
    # Build with skip_blocks=False first to initialise block weights
    spatial = ops.ones((1, in_dim, 16, 16))
    qf = ops.ones((1, 1, in_dim))
...
def test_segmentation_head_sparse():
    """Verify sparse_call returns matching intermediate components."""
    set_seed()
    in_dim = 32
    num_blocks = 1
