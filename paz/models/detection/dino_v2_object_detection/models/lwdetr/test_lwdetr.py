"""Unit tests for LWDETR model components.

Tests MLP, loss functions, LWDETR forward pass with various
configurations, PostProcess post-processing, and SetCriterion loss
computation using lightweight mock backbone and transformer.
"""
import sys
import os
...
class MockBackbone(keras.layers.Layer):
    """Lightweight backbone returning random multi-scale features.

    Returns 3 feature maps at progressively downsampled resolutions
    with corresponding zero masks and random position encodings.

    Args:
        hidden_dim (int): Feature channel dimensionality.
    """
...
class MockTransformer(keras.layers.Layer):
    """Lightweight transformer returning fixed-shape random decoder outputs.

    Produces deterministic output shapes: 6 decoder layers of
    (B, Q, d_model) hidden states with 4D reference points,
    plus encoder output hidden states and reference points.
    ...
    def __init__(self, d_model=256, num_queries=100):
        ...
        self.decoder = type('obj', (object,), {'bbox_embed': None})
...
def test_mlp():
    """Verifies MLP output shape matches expected dimensions."""
    input_dim = 16
    hidden_dim = 32
    output_dim = 4
    num_layers = 3
...
def test_lwdetr_instantiation():
    """Verifies LWDETR model construction and attribute assignment."""
    d_model = 256
    num_classes = 91
    num_queries = 100
...
def test_lwdetr_forward(group_detr, two_stage, lite_refpoint_refine, aux_loss):
    """Verifies LWDETR forward pass output shapes and structure."""
    d_model = 256
    num_classes = 91
    num_queries = 100
...
def test_lwdetr_various_inputs():
    """Verifies LWDETR handles different batch sizes and spatial dims."""
    d_model = 128
    num_classes = 10
    num_queries = 50
...
def test_postprocess():
    """Verifies PostProcess output shapes and coordinate scaling."""
    B, Q, C = 2, 100, 91
    num_select = 10
...
def test_criterion():
    """Verifies SetCriterion produces all expected loss keys."""
    num_classes = 91
