...
def build_detector_head(
    ...
    num_boxes = regressions.shape[-1] // num_dims
...
def ClassNet(
    ...
    return build_head(
        features, num_blocks, num_filters, bias_initializer, survival_rate
    )
...
def build_head(
    ...
    conv_blocks = build_head_conv2D(
        num_blocks, num_filters[0], keras.initializers.Zeros()
    )
...
def build_head_conv2D(num_blocks, num_filters, bias_initializer):
    ...
    """
    conv_blocks = []
    args_1 = (num_filters, 3, (1, 1), "same", "channels_last", (1, 1), 1, None, True)
