...
def MBconv_blocks(
    ...
    intro_filters = [
        scale_filters(intro_filter, W_coefficient, D_divisor)
        for intro_filter in intro_filters
    ]
    outro_filters = [
        scale_filters(outro_filter, W_coefficient, D_divisor)
...
def apply_drop_connect(x, is_training, survival_rate):
    ...
    if not is_training:
        ...
        random_tensor = survival_rate + keras.random.uniform(**kwargs)
