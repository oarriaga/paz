...
from .efficientdet_blocks import BiFPN, build_detector_head, EfficientNet_to_BiFPN
...
def EFFICIENTDET(
    ...
    branches, middles, skips = EfficientNet_to_BiFPN(EfficientNet, FPN_num_filters)
    ...
    if (base_weights == "COCO") and (head_weights == "COCO"):
        model_filename = "-".join(
            [model_name, str(base_weights), str(head_weights) + "_weights.hdf5"]
        )
    ...
    if not ((base_weights is None) and (head_weights is None)):
        ...
        finetunning_model_names = [
            "efficientdet-d0-COCO-None_weights.hdf5",
