...
def VVAD_LRS3_LSTM(weights=None, input_shape=(38, 96, 96, 3), seed=305865):
    """Binary Classification for videos using a CNN based mobile net with
    an TimeDistributed layer (LSTM).
    # Arguments
    ...
    random.seed(seed)
    initializer_glorot_lstm = tf.keras.initializers.GlorotUniform(
        seed=random.randint(0, 1000000))
    initializer_glorot_dense = tf.keras.initializers.GlorotUniform(
        seed=random.randint(0, 1000000))
    initializer_glorot_output = tf.keras.initializers.GlorotUniform(
        seed=random.randint(0, 1000000))
    initializer_orthogonal = tf.keras.initializers.Orthogonal(
        seed=random.randint(0, 1000000))
    ...
    x = LSTM(32, kernel_initializer=initializer_glorot_lstm,
             recurrent_initializer=initializer_orthogonal)(x)
    x = BatchNormalization()(x)

    x = Dense(512, activation='relu',
              kernel_initializer=initializer_glorot_dense)(x)
