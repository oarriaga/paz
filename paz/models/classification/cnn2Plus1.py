...
from tensorflow.keras.layers import (Conv3D, Input, BatchNormalization, Layer,
                                     GlobalAveragePooling3D, ReLU, Flatten,
                                     LayerNormalization, Dense, add)
from tensorflow.keras.utils import get_file
from tensorflow.keras.initializers import GlorotUniform


Architecture_Options = ['CNN2Plus1D', 'CNN2Plus1D_Filters', 'CNN2Plus1D_Layers',
                        'CNN2Plus1D_Light', 'CNN2Plus1D_18']
...
References:
    - [A Closer Look at Spatiotemporal Convolutions for Action 
        Recognition](https://arxiv.org/abs/1711.11248v3)
    - [Video classification with a 3D convolutional neural 
        network](https://www.tensorflow.org/tutorials/video/video_classification#load_and_preprocess_video_data)
"""
...
class Conv2Plus1D(Layer):
    def __init__(self, filters, kernel_size, padding):
        """
        A sequence of convolutional layers that first apply the convolution
        operation over the spatial dimensions, and then the temporal dimension.
        """
        super().__init__()
        initializer_glorot_spatial = tf.keras.initializers.GlorotUniform(
            seed=random.randint(0, 1000000))
        initializer_glorot_temporal = tf.keras.initializers.GlorotUniform(
            seed=random.randint(0, 1000000))
...
class Project(Layer):
    ...
    def __init__(self, units):
        super().__init__()
        initializer_glorot = GlorotUniform(
...
class ResizeVideo(Layer):
    ...
    def call(self, video):
        """Use tensorflow reshape to resize the tensor.
          ...
          # Arguments
            video: Tensor representation of the video, in the form of a set of
                frames.
        ...
        video_reshaped = tf.reshape(video, [-1, video_shape[2], video_shape[3],
                                            video_shape[4]])

        images_resized = self.resizing_layer(video_reshaped)

        videos = tf.reshape(images_resized,
                            [video_shape[0], video_shape[1], self.height,
                             self.width, video_shape[4]])
        return videos
...
def normal(input_layer, height, width):
    ...
    """
    x = (Conv2Plus1D(filters=16, kernel_size=(3, 7, 7), padding='same')
         (input_layer))
...
def filters(input_layer, height, width):
    ...
    x = (Conv2Plus1D(filters=32, kernel_size=(3, 7, 7), padding='same')
         (input_layer))
    x = BatchNormalization()(x)
    x = ReLU()(x)
    x = ResizeVideo(height // 2, width // 2)(x)
...
def CNN2Plus1D(weights=None, input_shape=(38, 96, 96, 3), seed=305865,
    ...
    # Arguments
        ...
        input_shape: List of integers. Input shape to the model in following
            format: (frames, height, width, channels) e.g. (38, 96, 96, 3).
        seed: Integer. Seed for random number generator.
        architecture: String. Name of the architecture to use.
            Currently supported: 'CNN2Plus1D', 'CNN2Plus1D_Filters',
            'CNN2Plus1D_Layers', 'CNN2Plus1D_Light'.
            'CNN2Plus1D_18' is only available without weights.
    ...
    random.seed(seed)
    initializer_glorot_output = GlorotUniform(
        seed=random.randint(0, 1_000_000))
    ...
    if architecture == 'CNN2Plus1D':
        ...
        if weights == 'VVAD_LRS3':
            ...
            weights_path = get_file(filename, URL + filename,
    ...
    x = Dense(1, activation='sigmoid',
              kernel_initializer=initializer_glorot_output)(x)
