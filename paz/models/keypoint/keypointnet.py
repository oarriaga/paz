...
from paz.models.keypoint.layers import ExpectedDepth
from paz.models.keypoint.layers import ExpectedValue2D
from paz.models.keypoint.layers import SubtractScalar
...
def KeypointNet2D(input_shape, num_keypoints, filters=64, alpha=0.1):
    ...
    uv_volume = Conv2D(num_keypoints, (3, 3), padding="same", name=name % 0)(
        base
    )
...
def KeypointNet(input_shape, num_keypoints, depth=0.2, filters=64, alpha=0.1):
    ...
    depth_volume = Conv2D(num_keypoints, (3, 3), padding="same", name=name % 0)(
        base
    )
...
def FaceKeypointNet2D32(draw=True, radius=3):
    model = KeypointNet2D((96, 96, 1), 15, 32, 0.1)
    URL = "https://github.com/oarriaga/altamira-data/releases/download/v0.7/"
