...
def test_extract_shape_list_of_arrays(sample_array):
    ...
    result = cache._extract_shape(arrays)
    assert result == [(3,), (3,)]
...
def test_extract_shape_nested_pytree(sample_array):
    ...
    result = cache._extract_shape(nested)
...
def test_extract_shape_mixed_pytree(sample_array):
    ...
    result = cache._extract_shape(mixed)
...
def test_key_same_shapes_produce_same_key(sample_array):
    ...
    key1 = cache._key(arr1)
    key2 = cache._key(arr2)
    assert key1 == key2
...
def test_key_pytree_structure_matters(sample_array):
    ...
    key1 = cache._key(dict_input)
    key2 = cache._key(list_input)
    assert key1 != key2
...
def test_jit_decorator_with_empty_args(temp_cache_dir):
    ...
    @cache.jit(cache_dir=temp_cache_dir)
    def add(x, y):
        return x + y
...
def test_jit_cache_filename_includes_module_name(temp_cache_dir):
    ...
    cache_files = list(temp_cache_dir.glob("*.bin"))
    assert len(cache_files) == 1
    assert "__main__" in cache_files[0].name or "cache_test" in cache_files[0].name
...
def test_jit_with_static_argnums(temp_cache_dir):
    ...
    @cache.jit(static_argnums=(1,), cache_dir=temp_cache_dir)
    def scale(x, factor):
        return x * factor
...
def test_jit_passes_kwargs_to_jax_jit(temp_cache_dir):
    ...
    @cache.jit(donate_argnums=(0,), cache_dir=temp_cache_dir)
    def add_one(x):
        return x + 1
...
def test_jit_multiple_arrays_input(temp_cache_dir):
    ...
    result = add_arrays(
        jp.array([1.0]), jp.array([2.0]), jp.array([3.0])
    )
...
def test_jit_same_function_name_different_modules(temp_cache_dir):
    """Same function name in different contexts should create different cache."""
