...
description = 'Training script for learning DPOD'
...
model = DPOD(input_shape=(args.image_size, args.image_size, 3), num_colors=255, num_objects=1)
optimizer = Adam(args.learning_rate, amsgrad=True)
model.compile(optimizer, args.loss, metrics=['mse'])
model.summary()

# setting scene
renderer = SingleView(filepath_original=args.obj_path,
                      filepath_uv_mapping=(args.image_size, args.image_size),
                      num_objects=1, y_fov=args.y_fov, distance=args.depth, light=args.light,
                      top_only=bool(args.top_only), roll=args.roll, shift=args.shift)
