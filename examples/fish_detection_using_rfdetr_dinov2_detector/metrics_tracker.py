...
class MetricsTracker:
    ...
    def generate_plots(self):
        ...
        # mAP@50
        ax1.plot(epochs, self.history["train_mAP_50"],
                 "b-o", markersize=3, label="Train mAP@50")
        ax1.plot(epochs, self.history["val_mAP_50"],
        ...
        # mAP@50:95
        ax2.plot(epochs, self.history["train_mAP_50_95"],
                 "b-o", markersize=3, label="Train mAP@50:95")
        ax2.plot(epochs, self.history["val_mAP_50_95"],
        ...
        ax1.grid(True, alpha=0.3)
        # Recall
        ax2.plot(epochs, self.history["train_recall"],
                 "b-o", markersize=3, label="Train")
        ax2.plot(epochs, self.history["val_recall"],
                 "r--s", markersize=3, label="Val")
        ax2.set_xlabel("Epoch")
        ...
        ax2.grid(True, alpha=0.3)
        # F1
        ax3.plot(epochs, self.history["train_f1"],
                 "b-o", markersize=3, label="Train")
        ax3.plot(epochs, self.history["val_f1"],
                 "r--s", markersize=3, label="Val")
        ...
        has_multi_lr = (
            ...
            and len(self.history.get("lr_backbone", [])) == len(epochs)
            and any(v > 0 for v in self.history.get("lr_backbone", []))
        )
        if has_multi_lr:
            ax.plot(epochs, self.history["lr_backbone"],
                    "b-.", markersize=2, label="Backbone LR", alpha=0.8)
            ax.plot(epochs, self.history["lr_decoder"],
                    "g-.", markersize=2, label="Decoder LR", alpha=0.8)
            ax.plot(epochs, self.history["lr_head"],
        ...
        has_grad = (
            ...
            and len(self.history.get("grad_norm", [])) == len(epochs)
        ...
        if has_grad:
            ...
            if "grad_norm_max" in self.history:
                ax.plot(epochs, self.history["grad_norm_max"],
        ...
        has_loss_comp = (
            ...
            and len(self.history.get("train_loss_ce", [])) == len(epochs)
            and any(v > 0 for v in self.history.get("train_loss_ce", []))
        )
        has_val_loss_comp = (
            "val_loss_ce" in self.history
            and len(self.history.get("val_loss_ce", [])) == len(epochs)
        ...
        if has_loss_comp:
            ...
            for i, (cname, ctitle) in enumerate(
                ...
                axes_lc[i].plot(
                    epochs, self.history[f"train_{cname}"],
                    "b-o", markersize=3, label=f"Train {cname}")
                if has_val_loss_comp:
                    axes_lc[i].plot(
                        epochs, self.history[f"val_{cname}"],
        ...
        axes[0, 1].plot(epochs, self.history["train_mAP_50"],
                        "b-o", markersize=2, label="Train mAP@50")
        axes[0, 1].plot(epochs, self.history["val_mAP_50"],
                        "r--s", markersize=2, label="Val mAP@50")
        axes[0, 1].plot(epochs, self.history["val_mAP_50_95"],
        ...
        axes[1, 0].plot(epochs, self.history["train_precision"],
                        "b-o", markersize=2, label="Train Prec")
        axes[1, 0].plot(epochs, self.history["val_precision"],
                        "r--s", markersize=2, label="Val Prec")
        axes[1, 0].plot(epochs, self.history["train_recall"],
                        "b-^", markersize=2, alpha=0.6, label="Train Rec")
        axes[1, 0].plot(epochs, self.history["val_recall"],
        ...
        # (1,1) F1 — Train vs Val
        axes[1, 1].plot(epochs, self.history["train_f1"],
                        "b-o", markersize=2, label="Train F1")
        axes[1, 1].plot(epochs, self.history["val_f1"],
        ...
        if has_loss_comp:
            axes[2, 0].plot(epochs, self.history["train_loss_ce"],
                            "b-.", markersize=2, label="train_ce")
            axes[2, 0].plot(epochs, self.history["train_loss_bbox"],
                            "r-.", markersize=2, label="train_bbox")
            axes[2, 0].plot(epochs, self.history["train_loss_giou"],
                            "g-.", markersize=2, label="train_giou")
            if has_val_loss_comp:
                axes[2, 0].plot(epochs, self.history["val_loss_ce"],
                                "b--", markersize=2, alpha=0.6,
                                label="val_ce")
                axes[2, 0].plot(epochs, self.history["val_loss_bbox"],
                                "r--", markersize=2, alpha=0.6,
                                label="val_bbox")
                axes[2, 0].plot(epochs, self.history["val_loss_giou"],
                                "g--", markersize=2, alpha=0.6,
                                label="val_giou")
            axes[2, 0].set_title("Loss Components (Train vs Val)")
            axes[2, 0].legend(fontsize=7, ncol=2)
        else:
            axes[2, 0].plot(epochs, self.history["val_accuracy"],
        ...
        # (2,1) Gradient norm
        if has_grad:
            axes[2, 1].plot(epochs, self.history["grad_norm"],
                            "darkorange", marker="o", markersize=2,
                            label="Avg")
            if "grad_norm_max" in self.history:
                axes[2, 1].plot(epochs, self.history["grad_norm_max"],
        ...
        if has_multi_lr:
            axes[3, 0].plot(epochs, self.history["lr_backbone"],
                            "b-.", markersize=2, label="Backbone")
            axes[3, 0].plot(epochs, self.history["lr_decoder"],
                            "g-.", markersize=2, label="Decoder")
            axes[3, 0].plot(epochs, self.history["lr_head"],
                            "r-o", markersize=2, label="Head")
            axes[3, 0].legend(fontsize=8)
        else:
            axes[3, 0].plot(epochs, self.history["learning_rate"],
                            "purple", marker=".", markersize=2)
        axes[3, 0].set_title("Learning Rate")
        axes[3, 0].grid(True, alpha=0.3)

        # (3,1) Accuracy — Train vs Val
        axes[3, 1].plot(epochs, self.history["train_accuracy"],
                        "b-o", markersize=2, label="Train")
        axes[3, 1].plot(epochs, self.history["val_accuracy"],
