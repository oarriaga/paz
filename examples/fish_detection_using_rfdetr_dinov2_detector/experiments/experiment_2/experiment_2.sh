#!/bin/bash
###############################################################################
# experiment_2.sh — Decoder-only training: RF-DETR Nano + DINOv2
#
# Trains the transformer decoder + detection head + query embeddings
# while freezing the DINOv2 backbone.  This is the recommended
# fine-tuning strategy when the backbone is well-pretrained (DINOv2).
#
# Key changes vs experiment_1:
#   - train_mode = decoder_only  (backbone frozen, decoder+head trainable)
#   - lr-component-decay = 0.7   (decoder gets 70% of head LR, RF-DETR default)
#   - clip-max-norm = 0.1        (RF-DETR default — was 1.0)
#   - confidence-threshold = 0.3 (saner evaluation — was 0.1)
#   - warmup_epochs = 5.0        (enough warmup for newly reinitialised head)
#   - group_detr = 13            (RF-DETR default; matches pretrained query embeddings)
#   - ema_decay = 0.993          (RF-DETR default — was 0.9997)
#   - ema_tau = 100.0            (RF-DETR default — was 2000.0)
#
...
#
# Usage:
#   sbatch experiment_2.sh
#   RESUME=1 sbatch experiment_2.sh
#
###############################################################################

#SBATCH --job-name=rfdetr_exp2_decoder_only
...
#SBATCH --output=/mnt/beegfs/home/mebrahim/projects/fish_detector_using_rfdetr/paz/examples/fish_detection_using_rfdetr_dinov2_detector/experiments/experiment_2/slurm_%j.out
#SBATCH --error=/mnt/beegfs/home/mebrahim/projects/fish_detector_using_rfdetr/paz/examples/fish_detection_using_rfdetr_dinov2_detector/experiments/experiment_2/slurm_%j.err
...
###############################################################################
EXP_BASE="/mnt/beegfs/home/mebrahim/projects/fish_detector_using_rfdetr/paz/examples/fish_detection_using_rfdetr_dinov2_detector/experiments/experiment_2"
...
EXP_DIR="${EXPERIMENTS_ROOT}/experiment_2"
...
###############################################################################
echo "============================================================"
echo "  EXPERIMENT 2: Decoder-only fine-tuning — RF-DETR Nano"
echo "============================================================"
echo "  Date           : $(date)"
echo "  Node           : $(hostname)"
...
cat > "${EXP_DIR}/experiment_config.json" <<EOF
{
    ...
    "description": "Decoder-only fine-tuning: RF-DETR Nano + DINOv2",
    "variant": "RFDETRNano",
...
echo ""
echo "============================================================"
echo "  EXPERIMENT 2 COMPLETE — exit code: ${EXIT_CODE}"
echo "  Date           : $(date)"
