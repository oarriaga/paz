...
from frame import extractFeatures, match_frames
...
class InitializeSFM(pr.Processor):
    ...
    def call(self, images):
        ...
        kps1, des1 = extractFeatures(image1)
        kps2, des2 = extractFeatures(image2)
        ...
        # twitchslam frature matcher ********************************

        indices1, indices2, essential_matrix = match_frames(kps1, des1,
                                                            kps2, des2)

        # compute fundamental matrix
        points1 = kps1[indices1]
        points2 = kps1[indices2]
        fundamental_matrix, mask = self.compute_fundamental_matrix(points1,
                                                                   points2)
        print('sdhfgajdsgfjs', fundamental_matrix.shape)

        # compute essential matrix
        essential_matrix = self.compute_essential_matrix(fundamental_matrix)
        print('4545454', essential_matrix.shape)
        ...
        kps2 = np.array(kps2)
        des2 = np.array(des2)
        return self.warp(points3D, [kps2[indices1],
                                    des2[indices1]], P2)
...
class StructureFromMotion(pr.Processor):
    ...
    def call(self, images):
        ...
        for arg in range(len(images)-2):
            ...
            keypoints1, descriptor1 = extractFeatures(images[arg + 1])
            keypoints2, descriptor2 = extractFeatures(images[arg + 2])
