...
from ransac2 import ransac, matcher
...
class InitializeSFM(pr.Processor):
    ...
    def call(self, images):
        ...
        matches = matcher(keypoints1, descriptor1,
                          keypoints2, descriptor2, 0.75)
        best_inliers, best_H, best_idx = ransac(matches, 0.5, 2000)
        # print('******************************************************')
        # print(best_inliers, best_H)
        # print('******************************************************')
        ...
        return self.warp(points3D, [keypoints2[best_idx],
                                    descriptor2[best_idx]], P2)
...
class StructureFromMotion(pr.Processor):
    ...
    def call(self, images):
        ...
        for arg in range(len(images)-2):
            ...
            matches = matcher(base_keypoints, base_descriptors,
                              keypoints2, descriptor2, 1.00)
            best_inliers, best_H, best_idx = ransac(matches, 5.5, 2000)
            points1 = best_inliers[:, 0:2]
            points2 = best_inliers[:, 2:4]


            _, rotation, translation = self.solve_pnp(points3d[best_idx],
                                                      points2)
            rotation = self.rotation_vector_to_matrix(rotation)
            P3 = rotation @ self.projection_matrix
            P3[:3, 3] = translation.ravel()
            ...
            keypoints2 = np.array(keypoints2)
            descriptor2 = np.array(descriptor2)
            base_features = [keypoints2[best_idx], descriptor2[best_idx]]
            print(P3)
            print('##################')
            P2 = P3.copy()
            # break
