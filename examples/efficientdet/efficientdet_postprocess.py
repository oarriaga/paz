...
def merge_class_box_level_outputs(cls_outputs, box_outputs,
...
def efficientdet_postprocess(cls_outputs, box_outputs, image_size,
                             image_scales, raw_images=None, min_level=3,
                             max_level=7, num_classes=90, num_scales=3,
                             aspect_ratios=[1.0, 2.0, 0.5], anchor_scale=4):
    coco = get_class_names('COCO_OFFICIAL')
    assert len(coco) == num_classes, "Number of classes not matching labels"
    num_levels = max_level - min_level + 1
    prior_boxes = get_prior_boxes(min_level, max_level, num_scales,
                                  aspect_ratios, anchor_scale, image_size)
