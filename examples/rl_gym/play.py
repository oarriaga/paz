...
def render_frames(mjmodel, positions, width, height, distance=3.0):
    ...
    # the near clipping plane scales with the model extent, which the 200 m
    # terrain pushes past the camera distance; pin it to a few centimetres
    mjmodel.vis.map.znear = 0.05 / mjmodel.stat.extent
...
if __name__ == "__main__":
    ...
    robot = G1DoF29()
    world = build_world(robot, terrain, args.backend, 1)
    indices = build_reward_indices(robot)
    max_level = world.terrain.origins.shape[0] - 1
    iteration = checkpoint.find_latest_iteration(args.checkpoint)
    actor = checkpoint.load_actor(args.checkpoint, iteration)
    reset = robust.build_batch_reset(world, world.dynamics, None)
    step_args = world, world.dynamics, None, indices, max_level
    step = jax.jit(robust.build_batch_step(*step_args))
