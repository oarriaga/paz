...
def test_diverged_samples_are_masked_from_the_loss():
    ...
    log_prob = jp.log(jp.array([1.0, 3.0]))
    loss = float(ppo.compute_policy_loss(log_prob, batch, clip_ratio=0.2))
    assert np.isclose(loss, -1.0, atol=1e-5)
...
def test_shuffle_preserves_and_permutes_each_shard():
    ...
    sharded = ppo.split_shards(experience, 2)
    shuffled = ppo.shuffle_shards(jr.key(3), sharded)
    for shard in range(2):
        original = np.sort(np.asarray(sharded.action[shard]))
        permuted = np.sort(np.asarray(shuffled.action[shard]))
        assert np.allclose(original, permuted)


def test_split_and_merge_batches():
    experience = build_batch(8, action=jp.arange(8.0))
    sharded = ppo.split_shards(experience, 2)
    batches = ppo.split_batches(sharded, 2)
    assert batches.action.shape == (2, 2, 2)
    merged = ppo.merge_shards(batches.action[0])
    assert np.allclose(np.asarray(merged), [0.0, 1.0, 4.0, 5.0])
