...
def actuator_gain(key, model, num_joints, minval=0.8, maxval=1.2):
    scale = jr.uniform(key, (num_joints,), minval=minval, maxval=maxval)
    return model.actuator_gainprm[:, 0] * scale


def actuator_velocity_damping(key, model, num_joints, minval=0.8, maxval=1.2):  # fmt: skip
    scale = jr.uniform(key, (num_joints,), minval=minval, maxval=maxval)
...
def armature(key, model, num_joints, minval=0.0, maxval=0.005):
