...
Terrain = namedtuple("Terrain", "heights, origins, tile_size, border_width, horizontal_scale")  # fmt: skip
...
def build(seed, num_levels=9, tile_size=8.0, border_width=20.0, horizontal_scale=0.1, vertical_scale=0.005, counts=TERRAIN_COUNTS):  # fmt: skip
    ...
    for level in range(num_levels):
        for column, terrain_type in enumerate(terrain_types):
            # the reference draws every tile's difficulty inside its level band
    ...
    return Terrain(heights, origins, tile_size, border_width, horizontal_scale)
...
def add_heightfield(model_spec, terrain):
    # one heightfield per tile, each in its own frame: mjx warp collides
    # heightfields in float32 in the field frame, and one 200 m field loses
    # the sub-millimetre precision the 5 mm foot spheres need far from its
    # origin, exploding joints at 0.5% of steps on the outer tiles
    hide_floor(model_spec)
    material = add_terrain_material(model_spec)
    tile_cells = compute_num_cells(terrain.tile_size, terrain.horizontal_scale)  # fmt: skip
    border_cells = compute_num_cells(terrain.border_width, terrain.horizontal_scale)  # fmt: skip
    num_levels, num_columns = terrain.origins.shape[:2]
    for level in range(num_levels):
        for column in range(num_columns):
            region = compute_tile_region(level, column, tile_cells, border_cells)  # fmt: skip
            origin = terrain.origins[level, column]
            tile_args = model_spec, terrain.heights[region], origin, terrain.tile_size, f"tile_{level}_{column}"  # fmt: skip
            add_tile(*tile_args).material = material
    add_border(model_spec, terrain)


def hide_floor(model_spec):
...
def add_tile(model_spec, heights, origin, tile_size, name, thickness=0.1, vertical_scale=0.005):  # fmt: skip
    minimum = float(heights.min())
    peak = max(float(heights.max()) - minimum, vertical_scale)
    size = [tile_size / 2, tile_size / 2, peak, thickness]
    field = model_spec.add_hfield(name=name, size=size)
    field.nrow, field.ncol = heights.shape
    field.userdata = ((heights - minimum) / peak).ravel()
    geom = model_spec.worldbody.add_geom()
    geom.name = name
    geom.type = mujoco.mjtGeom.mjGEOM_HFIELD
    geom.hfieldname = name
    geom.pos = [origin[0], origin[1], minimum]
    return geom


def add_border(model_spec, terrain, thickness=0.1):
    # the flat frame around the tiles as four thin boxes
    rows, columns = terrain.heights.shape
    width_x = (columns - 1) * terrain.horizontal_scale
    width_y = (rows - 1) * terrain.horizontal_scale
    inner_x = width_x / 2 - terrain.border_width
    inner_y = width_y / 2 - terrain.border_width
    half = terrain.border_width / 2
    strips = [(0.0, inner_y + half, width_x / 2, half), (0.0, -inner_y - half, width_x / 2, half), (inner_x + half, 0.0, half, inner_y), (-inner_x - half, 0.0, half, inner_y)]  # fmt: skip
    for x, y, half_x, half_y in strips:
        geom = model_spec.worldbody.add_geom()
        geom.type = mujoco.mjtGeom.mjGEOM_BOX
        geom.size = [half_x, half_y, thickness]
        geom.pos = [x, y, -thickness]
...
def draw(terrain, counts=TERRAIN_COUNTS):
    crop_args = terrain.heights, terrain.horizontal_scale, terrain.border_width
...
def label_levels(axis, num_levels):
    labels = []
    for level in range(num_levels):
        labels.append(f"{level / num_levels:.2f}")
