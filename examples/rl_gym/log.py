...
FIELDS = "iteration, reward, episode_return, tracking, loss, value_loss, entropy, KL, gradient_norm, learning_rate, max_speed, steps_per_second"  # fmt: skip
...
def build_row(iteration, metrics, update_metrics, max_speed, steps, elapsed):
    ...
    args = iteration, float(metrics.reward), float(metrics.episode_return), float(metrics.terms[0]), update.loss, update.value_loss, update.entropy, update.KL, update.gradient_norm, update.learning_rate, float(max_speed), steps / elapsed  # fmt: skip
    return Row(*args)
