...
class SCR(nn.Module):
    def __init__(
        ...
        padding1 = (
            (0, self.ksize[2] // 2, self.ksize[3] // 2)
            if do_padding
            else (0, 0, 0)
        )
        ...
        self.conv1x1_in = nn.Sequential(
            nn.Conv2d(
                planes[0], planes[1], kernel_size=1, bias=False, padding=0
            ),
        ...
        self.conv1x1_out = nn.Sequential(
            nn.Conv2d(
                planes[3], planes[4], kernel_size=1, bias=False, padding=0
            ),
...
class CCALayer(nn.Module):
    ...
    def forward(self, spt, qry):
        ...
        similarity_matrix = F.cosine_similarity(
            spt_attended, qry_attended, dim=-1
        )
