...
class GeneratedImageProcessor(Processor):
    ...
    """
    def __init__(self, path_images, background_images_paths, num_occlusions=1, split=pr.TRAIN, num_stages=6, no_ambiguities=False, multipleHypotheses=None):
    ...
    def call(self):
        ...
        if self.num_stages == 1:
            return image_original, belief_maps

        if self.num_stages == 2:
            return image_original, belief_maps, belief_maps

        if self.num_stages == 3:
            return image_original, belief_maps, belief_maps, belief_maps

        if self.num_stages == 4:
            return image_original, belief_maps, belief_maps, belief_maps, belief_maps

        if self.num_stages == 5:
            return image_original, belief_maps, belief_maps, belief_maps, belief_maps, belief_maps

        return tuple([image_original] + [belief_maps]*(6*self.multipleHypotheses.M))


class GeneratedImageGenerator(SequentialProcessor):
    def __init__(self, path_images, background_images_paths, num_occlusions, image_size_input, image_size_output, num_stages, split=pr.TRAIN, multipleHypotheses=None):
        super(GeneratedImageGenerator, self).__init__()
        self.add(GeneratedImageProcessor(
            path_images, background_images_paths, num_occlusions, num_stages=num_stages, split=split, multipleHypotheses=multipleHypotheses))

        outputs_raw = dict()
        outputs = dict()
        for i in range(1, num_stages+1):
            output_belief_maps = multipleHypotheses.map_layer_names_to_attributes({'belief_maps_stage_' + str(i): [image_size_output, image_size_output, 9]})
            outputs_raw = {**outputs_raw, **output_belief_maps}

        for i, key in enumerate(outputs_raw):
            outputs[i+1] = {key: outputs_raw[key]}
