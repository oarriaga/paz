...
def build_detector_head(middles, num_classes, num_dims, aspect_ratios,
    ...
    args = (middles, momentum, epsilon, num_anchors,
    ...
    classes = Activation(activation)(classes)
    regressions = Reshape((num_boxes, num_dims))(regressions)
    outputs = Concatenate(axis=2, name='boxes')([regressions, classes])
    return outputs


def ClassNet(features, momentum, epsilon, num_anchors=9, num_filters=32,
             num_blocks=4, survival_rate=None, num_classes=90):
...
def BoxesNet(features, momentum, epsilon, num_anchors=9, num_filters=32,
             num_blocks=4, survival_rate=None, num_dims=4):
    """Initializes BoxNet.
...
def EfficientNet_to_BiFPN(branches, num_filters,
    ...
    """
    args = num_filters, momentum, epsilon
    branches = extend_branch(branches, *args)
    P3, P4, P5, P6, P7 = branches
    P3_middle = conv_batchnorm_block(P3, *args)
    P4_middle = conv_batchnorm_block(P4, *args)
    P5_middle = conv_batchnorm_block(P5, *args)
    middles = [P3_middle, P4_middle, P5_middle, P6, P7]

    P4_skip = conv_batchnorm_block(P4, *args)
    P5_skip = conv_batchnorm_block(P5, *args)
    skips = [None, P4_skip, P5_skip, P6, None]
    return [branches, middles, skips]


def extend_branch(branches, num_filters, momentum, epsilon):
    ...
    P6, P7 = build_branch(P5, num_filters, momentum, epsilon)
    branches_extended = [P3, P4, P5, P6, P7]
    return branches_extended


def build_branch(P5, num_filters, momentum, epsilon):
    ...
    """
    P6 = conv_batchnorm_block(P5, num_filters, momentum, epsilon)
    P6 = MaxPooling2D(3, 2, 'same')(P6)
    P7 = MaxPooling2D(3, 2, 'same')(P6)
    return [P6, P7]


def conv_batchnorm_block(x, num_filters, momentum, epsilon):
...
def BiFPN(middles, skips, num_filters, fusion, momentum=0.99, epsilon=0.001):
    """BiFPN block.
    ...
    # Downpropagation ---------------------------------------------------------
    args = (num_filters, fusion, momentum, epsilon)
    ...
    middles = [P3_out, P4_top_down, P5_top_down, P6_top_down, P7_out]
    return [middles, middles]


def node_BiFPN(up, middle, down, skip, num_filters, fusion, momentum, epsilon):
    ...
    middle = BatchNormalization(momentum=momentum, epsilon=epsilon)(middle)
    return middle
