...
class SimpleBaselines3D(pr.Processor):
	def __init__(self, model, data_mean2D, data_stdev2D, data_mean3D,
		...
		self.model = model
		self.filter = pr.SequentialProcessor()
		self.filter.add(FilterKeypoints2D())
		self.preprocess = pr.SequentialProcessor()
		self.preprocess.add(StandardizeKeypoints2D(data_mean2D, data_stdev2D))
		self.postprocess = pr.SequentialProcessor()
		self.postprocess.add(
			UnnormalizeData(data_mean3D, data_stdev3D, dim_to_use3D))

	def call(self, keypoints2D):
		keypoints2D = self.filter(keypoints2D)
		normalized_data = self.preprocess(keypoints2D)
		keypoints3D = self.model.predict(normalized_data)
