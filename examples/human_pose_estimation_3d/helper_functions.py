...
import viz
from human36m import filter_keypoints3D


def compute_joints_distance(initial_translation, keypoints3D,
	...
	project2D = project_3D_to_2D(new_poses3D.reshape((-1, 3)), focal_length,
	                          image_center)
	joints_distance = np.linalg.norm(np.ravel(keypoints2D) - np.ravel(project2D))
...
def solve_translation(keypoints2D, keypoints3D, image_height, image_width):
	...
	"""
	joints3D = filter_keypoints3D(keypoints3D)
	...
	for i in range(keypoints3D.shape[0]):
		...
		points = project_3D_to_2D(keypoints3D[i].reshape((-1, 3)),
		                          focal_length,
		                          image_center)
		optimezed_poses3D.append(np.reshape(points, [1, 64]))
	return keypoints2D, joints3D, keypoints3D, np.array(optimezed_poses3D)
...
def project_3D_to_2D(points3D, focal_length, image_center):
	...
	# Arguments
		points3D: Nx3 points in camera coordinates
	...
	"""
	z = points3D[:, 2]
	u = (focal_length / z) * points3D[:, 0] + image_center[0, 0]
	v = (focal_length / z) * points3D[:, 1] + image_center[0, 1]
...
def get_bones_length(poses2D, poses3D):
	...
	#Arguments
		poses3D: array of predicted poses in 3D
		poses2D: array of poses in 2D
...
def initialize_translation(focal_length, joints2D, image_center, ratio):
	...
	x = (joints2D[:, 0] - camera_center_X) * ratio
	y = (joints2D[:, 1] - camera_center_Y) * ratio
