...
class AugmentDetection(SequentialProcessor):
    ...
    def __init__(self, prior_boxes, split=pr.TRAIN, num_classes=21, size=300,
                 mean=pr.BGR_IMAGENET_MEAN, IOU=.5, variances=[0.1, 0.1, 0.2, 0.2], background_image_paths=""):
        super(AugmentDetection, self).__init__()

        # image processors
        self.augment_image = AugmentImage()
        self.augment_background = RandomizeRenderedImage(image_paths=background_image_paths, num_occlusions=0)
        ...
        # pipeline
        self.add(pr.UnpackDictionary(['image', 'boxes', 'alpha_mask']))
        self.add(pr.ControlMap(self.augment_background, [0, 2], [0, 2]))
