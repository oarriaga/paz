...
from examples.dino_object_detection.models.utils import (
    torch_box_ops_for_testing as box_ops,
)
...
from examples.dino_object_detection.models.matcher.torch_matcher_for_testing import (
    build_matcher,
)
...
def build_model(args):
    # 1. Class Logic
    # COCO has max_obj_id 90, so we pass num_classes=91.
    num_classes = args.num_classes + 1

    # 2. Target Shape Logic
    ...
    # 3. Build Backbone
    # Ensures compatibility whether build_backbone returns a tuple (model, pos_enc) or just model
    ...
    # 4. Handle Encoder/Backbone only modes
    if args.encoder_only:
        # Handle case where backbone is (model, position_encoding)
        if isinstance(backbone, (tuple, list)):
            backbone_net = backbone[0]
        else:
            backbone_net = backbone

        # If the backbone is a custom class with an .encoder attribute, return that.
        # Otherwise (e.g. standard Keras Functional model), return the net itself.
        if hasattr(backbone_net, "encoder"):
            return backbone_net.encoder, None, None
        else:
            return backbone_net, None, None
...
def build_criterion_and_postprocessors(args):
    ...
    # 2. Safe attribute access for Group DETR logic
    sum_group_losses = getattr(args, "sum_group_losses", False)

    # 3. Configure Criterion kwargs
    criterion_kwargs = {
        "num_classes": args.num_classes + 1,
        "matcher": matcher,
    ...
    # Add segmentation specific args only if head is present
    if args.segmentation_head:
        criterion_kwargs["mask_point_sample_ratio"] = args.mask_point_sample_ratio

    # 4. Instantiate Criterion
