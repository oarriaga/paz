...
class FaceClassifier(Processor):
    ...
    def __call__(self, image):
        ...
        x = pr.Predict(self.detector, 'image', 'boxes', processors)(x)
        x = pr.ToBoxes2D()(x)
        x = pr.CropBoxes2D(self.offsets)(x)
        for image, box2D in zip(x['image_crops'], x['boxes2D']):
            i = pr.ConvertColor('BGR', 'GRAY')({'image': image})
            i = pr.NormalizeImage()(i)
            i = pr.Resize(self.classifier.input_shape[1:3])(i)
            i = pr.Lambda(np.expand_dims, {'axis': 0}, 'image')(i)
            i = pr.Lambda(np.expand_dims, {'axis': 3}, 'image')(i)
            i = pr.Predict(self.classifier, 'image')(i)
            i = pr.ToLabel(self.class_names)(i)
            box2D.class_name = i['predictions']
        x = pr.DrawBoxes2D(class_names)(x)
        x = pr.CastImageToInts()(x)
        return x


class FaceClassifierV2(Processor):
    def __init__(self, detector, classifier, offsets, class_names):
        processors = [pr.ConvertColor('BGR', 'GRAY')]
        self.detect = pr.Predict(detector, 'image', 'boxes', processors)
        self.to_boxes = pr.ToBoxes2D()
        self.crop_boxes = pr.CropBoxes2D(offsets)
...
pipeline = FaceClassifierV2(detector, classifier, offsets, class_names)
