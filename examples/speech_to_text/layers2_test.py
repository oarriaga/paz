...
from examples.speech_to_text.layers import build_fixed_length_waveform as build_fixed_length_waveform_old
from examples.speech_to_text.layers import build_decoder_self_attention_mask as build_decoder_self_attention_mask_old
from examples.speech_to_text.layers import build_whisper_mel_filters
...
from examples.speech_to_text.layers2.embedding import position_embedding
from examples.speech_to_text.layers2.embedding import token_and_position_embedding
from examples.speech_to_text.layers2.encoder import encoder_block
from examples.speech_to_text.layers2.decoder import decoder_block
from examples.speech_to_text.layers2.frontend import build_fixed_length_waveform
from examples.speech_to_text.layers2.frontend import frontend
from examples.speech_to_text.layers2.mel_spectrogram import build_mel_filters
...
def build_weight_suffix(path):
    ...
    if len(parts) <= 2:
        return path
    if parts[0] in WRAPPER_MODEL_NAMES:
        return "/".join(parts[1:])
    if parts[1] in ATTENTION_PROJECTION_NAMES:
        return path
    return "/".join(parts[1:])
...
def build_encoder_block_model():
    ...
    outputs = encoder_block(hidden, 2, 8, 0.0, 1e-5, "float32")
