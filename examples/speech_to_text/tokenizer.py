...
def load_tokenizer_config(config_path=None):
    if config_path is None:
        config_path = build_tokenizer_config_path()
...
def text_to_bytes(token_text, char_to_byte):
    return bytes(char_to_byte[c] for c in token_text)


def build_token_id_to_bytes(vocab_path=None):
    if vocab_path is None:
        vocab_path = build_vocabulary_path()
...
def decode_whisper_tokens(token_ids, vocab_path=None, config_path=None):
    id_to_bytes = build_token_id_to_bytes(vocab_path)
