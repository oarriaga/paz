...
def enforce_rank_2(affine_matrix):
    # Enforce rank-2 by performing SVD and zeroing out smallest singular value
    U, S, V = jp.linalg.svd(affine_matrix)
    S = S.at[-1].set(0)
    affine_matrix = U @ jp.diag(S) @ V
...
def compute_fundamental_matrix(points2D_1, points2D_2):
    ...
    F_normalized = enforce_rank_2(F_normalized)
...
def compute_essential_matrix(fundamental_matrix, camera_intrinsics):
    """Compute essential with fundamental matrix and camera intrinsics."""
