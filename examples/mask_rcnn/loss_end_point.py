...
class ProposalClassLoss(tf.keras.layers.Layer):
    ...
    def call(self, y_true, y_pred):
        ...
        self.add_loss(tf.reduce_mean(input_tensor=loss, keepdims=True)
                      * self.config.LOSS_WEIGHTS.get('rpn_class_loss', 1.))
        metric = (loss * self.config.LOSS_WEIGHTS.get('rpn_class_loss', 1.))
...
class ProposalBBoxLoss(tf.keras.layers.Layer):
    ...
    def call(self, y_true, y_pred):
        input_rpn_bbox = y_true[:, self.config.RPN_ANCHORS_PER_IMAGE, :]
        rpn_match = y_true[:, int(y_true.shape[1] / 2):, :1]
        ...
        metric = (loss * self.config.LOSS_WEIGHTS.get('rpn_bbox_loss', 1.))
...
class ClassLoss(tf.keras.layers.Layer):
    ...
    def call(self, y_true, y_pred):
        ...
        loss = tf.reduce_sum(loss) / (tf.math.reduce_sum(input_tensor=tf.cast(pred_active, 'float32')) + tf.constant(self.eps))
        self.add_loss(loss * self.config.LOSS_WEIGHTS.get('mrcnn_class_loss', 1.))
        metric = (loss * self.config.LOSS_WEIGHTS.get('mrcnn_class_loss', 1.))
...
class BBoxLoss(tf.keras.layers.Layer):
    ...
    """
    def __init__(self, config, target_class_ids, name='mrcnn_bbox_loss'):
        self.config = config
    ...
    def call(self, y_true, y_pred):
        ...
        metric = (loss * self.config.LOSS_WEIGHTS.get('mrcnn_bbox_loss', 1.))
...
class MaskLoss(tf.keras.layers.Layer):
    ...
    """
    def __init__(self, config, target_class_ids, name='mrcnn_mask_loss'):
        self.config = config
        self.target_class_ids = target_class_ids
    ...
    def call(self, y_true, y_pred):
        ...
        metric = (loss * self.config.LOSS_WEIGHTS.get('mrcnn_mask_loss', 1.))
