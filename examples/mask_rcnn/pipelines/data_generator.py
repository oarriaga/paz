    ...
    def __init__(self, image_shape, anchor_scales, backbone,
        ...
        self.add(SequenceWrapper(
            {0: {'input_image': [image_shape[0], image_shape[1],
                                 image_shape[2]]},
             1: {'input_gt_class_ids': [100]},
             2: {'input_gt_boxes': [100, 4]},
             3: {'input_gt_masks': [128, 128, 100]}},
            {4: {'rpn_class_logits': [4092, 1]},
             5: {'rpn_bounding_box': [4348, 4]}}))
...
class ConcatenateRPNValues(Processor):
    ...
    def call(self, RPN_bounding_box, RPN_match, anchors_shape):
        zeros_array = np.zeros((anchors_shape, 3))
        RPN_bounding_box_padded = np.zeros((256, 4))
        ...
        RPN_values = np.concatenate((RPN_bounding_box_padded, RPN_match_padded
                                     ))
        return RPN_values
...
class SubtractMeanImage(Processor):  # TODO: use paz function
    """Given the anchors and Ground truth boxes, compute overlaps and identify
    positive anchors and deltas to refine them to match their corresponding
    Ground truth boxes.
    ...
    def call(self, image):
        return subtract_mean_image(image, self.mean)
