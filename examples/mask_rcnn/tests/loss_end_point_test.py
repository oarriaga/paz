...
def test_class_loss(config, class_ids, softmax_loss):
    ...
    class_loss = ClassLoss(config=config)(y_true, class_ids)
    if session._closed:
...
def test_rpn_box_loss(config, target_RPN_boxes, RPN_boxes, l1_loss):
    ...
    box_loss = ProposalBBoxLoss(config=config)\
        (RPN_match,RPN_boxes)
    if session._closed:
...
def test_mrcnn_box_loss(config, target_boxes, boxes, l1_loss):
    ...
    box_loss = BBoxLoss(config=config)([target_ids, target_boxes], boxes)
    if session._closed:
        box_loss = box_loss.numpy()
    else:
        box_loss = session.run(box_loss)
